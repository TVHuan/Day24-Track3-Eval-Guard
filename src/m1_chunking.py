...
def chunk_semantic(text: str, threshold: float = SEMANTIC_THRESHOLD,
    ...
    for i in range(1, len(sentences)):
        sim = dot(embeddings[i-1], embeddings[i]) / (norm(embeddings[i-1]) * norm(embeddings[i]) + 1e-9)
        if sim < threshold:
